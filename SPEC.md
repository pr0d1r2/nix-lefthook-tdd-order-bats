...
| B4 | 2026-07-29 | `nix flake update` pulled `set-and-setting` rev that dropped `lib` output — `attribute 'lib' missing` on eval; lock file also exceeded 65 KB size limit (179 KB with duplicate inputs) | changed `set-and-setting` input to `follows = "nixpkgs-lock/set-and-setting"` (deduplicates inputs, locks to tested rev); raised `.lock` size limit to 128 KB |
| B5 | 2026-09-26 | devShell omitted `bats`, so the documented and CI-invoked `nix develop -c bats tests/unit/` command failed with exit 127 | added `pkgs.bats` to devShell `basePackages` |
